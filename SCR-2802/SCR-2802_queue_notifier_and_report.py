...
<parameter>
    ...
    <parameter>
     ...
     <parameter>
        <id>CLEAR_SCREEN</id>
        <name>Удалять скриншоты старше 7 дней</name>
        <type>boolean</type>
        <value>False</value>
    </parameter>
    <parameter>
...
class Reporter:
    ...
    def reports_folder():
        ...
        try:
            ...
            logger.debug('Folder allready exists: %s' % err)
    ...
    def _prepare_report_params(self, today_str):
        ...
        report_days = {"daily": [yesterday.isoformat()]}
    ...
    def generate_reports(self, today_str, wide_range=False):
        ...
        "" Если wide_range == True, то отчет формируется за предыдущие сутки
         +  всё что накопилось на текущий день
        ...
        try:
            if not wide_range:
                ...
                logger.debug('report_days["daily"]: %s' % report_days.get('daily', 'No info'))

            else:
                """
                makes report for all events of yesterday and today from 00-00 to current time
                """
                yesterday = datetime.now().date()
                report_data = {'daily_plus': {datetime.now().date().isoformat(): self._wide_range_data_loader()}}

            reports = [self._make_xlsx(report_type, report_data, yesterday.isoformat())
                       for report_type, report_data in report_data.iteritems()]
            logger.debug('wide_range: %s, len(reports): %s' % (wide_range, len(reports)))
...
class ChanEvents:
    """
    Класс контейнер хранит информацию от Object Entered the Zone по нужному каналу и зоне:
    время детекции, кол-во объектов
    ...
    def get_chan_zone(self):
        if self.channel and self.zone_guid:
...
class DeepDetectionHandler:
    ...
    def handler(self, event):
        ...
        detected_objects_quantity = int(event.p2)
        if not detected_objects_quantity:
            return
        logger.debug('add detection: chan: %s, zone: %s, p2: %s, p1: %s ' % (
            object(channel_guid).name,
            zone_name,
            event.p2,
            event.p1))
...
class DeepDetectionsWatcher:
    ...
    def check_chain_of_detections(self, detections, detections_instance):
        ...
        logger.debug('original detections: %s' % detections)
        ...
        for event_pair in _detections:
            ...
            if objects_quantity >= max_quantity:
                ...
                logger.debug('objects quantity is: {}, max quantity: {}, duration: {}, chan_name: {}, zone_name: {}'.format(
                                 ...
                                 BaseUtils.get_object_name_by_guid(zone_guid)
                             ))
...
class MakeAction:
    """
    storage_file_message[image_path] = (subject, alarm_message)
    ...
    screenshots_folder = os.path.join(BaseUtils.get_screenshot_folder(), screenshots_sub_folder)
    ...
    def delete_old_screenshots():
        _screenshot_folder = unicode(MakeAction.screenshots_folder)
        current_timestamp = int(time.time())
        all_files = os.listdir(_screenshot_folder)
        if not all_files:
            return

        files_lifitime = [(x, current_timestamp - MakeAction.creation_date(os.path.join(_screenshot_folder, x)))
                              for x in all_files]

        storage_time = SCREEN_SHOT_STORAGE_TIME * 24 * 60 * 60

        for file_name, life_time_seconds in files_lifitime:
            if life_time_seconds > storage_time:
                full_path = os.path.join(_screenshot_folder, file_name)
                try:
                    os.remove(full_path)
                except Exception as err:
                    logger.error("unable to delete file: %s. Error: %s" % (full_path, err))
    ...
    def make_text(ts, channel_name, zone_name, people_quantity):
        alarm_message = "Превышение людей в очереди!\n" + \
        ...
        logger.debug('alarm_message %s' % alarm_message)
    ...
    @staticmethod
    def send_email(alarm_message, subject="Максимальное количество людей в очереди", image_path=None):
    ...
    @staticmethod
    def make_file_name(channel_name, zone_name, dt=None):
        if dt is None:
            dt = time.time()
        event_date = datetime.fromtimestamp(dt)
        event_date_str_represent = event_date.strftime("%Y.%m.%d %H-%M-%S")
        file_name = "{}_{} ({}).jpg".format(channel_name, zone_name, event_date_str_represent)
    ...
    def process_ev(ev):
        ...
        if ev.type == "max people exceeded":
            ...
            channel_name = BaseUtils.get_object_name_by_guid(ev.channel_guid)
            zone_name = BaseUtils.get_object_name_by_guid(ev.zone_guid)
            ...
            file_name = MakeAction.make_file_name(channel_name, zone_name, dt=ts)
            logger.debug("file_name %s" % file_name)
            ...
            if SEND_SMS:
                sms_sender.text(shot_alarm_message)
            if CLEAR_SCREEN:
...
def make_wide_range_report():
    ...
    if object(SCHEDULE).state("color") == colors[SCHEDULE_COLOR]:
        logger.debug('Generate report by schedule')
        date_now = datetime.now().date().isoformat()
        reporter.generate_reports(date_now, wide_range=True)
...
SCREEN_SHOT_STORAGE_TIME = 7 # Screenshots older than SCREEN_SHOT_STORAGE_TIME days deletes in screenshot folder
...
if SEND_SMS:
    assert SMSC_LOGIN, 'Необходимо указать логин от сервиса https://smsc.ru/!'
    assert SMSC_PASSWORD, 'Необходимо указать пароль от сервиса https://smsc.ru/!'
    assert PHONE_NUMB, 'Необъодимо указать номер телефона для СМС!'
